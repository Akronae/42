...
{
	if (type == POINTER)
		elem->value = ft_arg_ptr_to_str(va_arg(args, t_ullong));
	else if (type == U_DECIMAL)
		elem->value = ft_number_to_str(va_arg(args, t_uint));
	else if (type == PERCENT)
		elem->value = ft_strdup("%");
	else if (type == LONGLONG)
		elem->value = ft_number_to_str(va_arg(args, t_llong));
	else if (type == HEXADECIMAL || type == UPPER_HEXADECIMAL)
		elem->value = ft_arg_hex_to_str(va_arg(args, t_uint), type);
}
