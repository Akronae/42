...
t_buffer	*new_buffer_2(t_buffer *buff)
{
	buff->write_int = ft_buffer_write_int;
	buff->write_kv_pair = ft_buffer_write_kv_pair;
...
t_buffer	*new_buffer(void)
{
	...
	buff = ft_safe_malloc(sizeof(t_buffer));
	buff->data = NULL;
	buff->size_bits = 0;
	buff->index_write = 0;
	buff->index_read = 0;
	...
	buff->read_kv_pair = ft_buffer_read_kv_pair;
