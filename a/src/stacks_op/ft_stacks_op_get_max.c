...
long long	ft_stacks_op_get_max(t_stacks_op *self)
{
	...
	if (self->stack_a->length == 0)
		return (*self->stack_b->find_max(self->stack_b, T_TYPE_LONG)
			->data->as_long);
	if (self->stack_b->length == 0)
		return (*self->stack_a->find_max(self->stack_a, T_TYPE_LONG)
			->data->as_long);
	return (ft_math_max(*self->stack_a->find_max(self->stack_a, T_TYPE_LONG)
			->data->as_long, *self->stack_b
			->find_max(self->stack_b, T_TYPE_LONG)
			->data->as_long));
}
